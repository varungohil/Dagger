...
class RDMA {
 public:
  ...
  RDMA(uint64_t base_nic_addr, size_t max_qp_pool_size)
...
#    pragma message "compiling Nic to run in polling mode"
    // Simple case so far: number of NIC flows = max_qp_pool_size_.
    nic_ = std::unique_ptr<Nic>(
        new NicPollingCCIP(base_nic_addr_, max_qp_pool_size_, false));
#  elif NIC_CCIP_MMIO
...
#    pragma message "compiling Nic to run in MMIO mode"
    // Simple case so far: number of NIC flows = max_qp_pool_size_.
    nic_ = std::unique_ptr<Nic>(
        new NicMmioCCIP(base_nic_addr_, max_qp_pool_size_, false));
#  elif NIC_CCIP_DMA
#    pragma message "compiling Nic to run in DMA mode"
    // Simple case so far: number of NIC flows = max_qp_pool_size_.
    nic_ = std::unique_ptr<Nic>(
        new NicDmaCCIP(base_nic_addr_, max_qp_pool_size_, false));
#  else
...
#    pragma message "compiling Nic to run in polling mode"
    // Simple case so far: number of NIC flows = max_qp_pool_size_
    nic_ = std::unique_ptr<Nic>(
        new NicPollingCCIP(base_nic_addr_, max_qp_pool_size_, true));
#  elif NIC_CCIP_MMIO
// MMIO intefrace only works either with write-combine buffering or AVX
// intrinsics
#    pragma message "compiling Nic to run in MMIO mode"
    // Simple case so far: number of NIC flows = max_qp_pool_size_
    nic_ = std::unique_ptr<Nic>(
        new NicMmioCCIP(base_nic_addr_, max_qp_pool_size_, true));
#  elif NIC_CCIP_DMA
#    pragma message "compiling Nic to run in DMA mode"
    // Simple case so far: number of NIC flows = max_qp_pool_size_
    nic_ = std::unique_ptr<Nic>(
        new NicDmaCCIP(base_nic_addr_, max_qp_pool_size_, true));
#  else
#    error Nic CCI-P mode is not specified
#  endif
  ...
  T* pop() {
    ...
    if (qp_pool_cnt_ < max_qp_pool_size_) {
      ...
      qp_pool.push_back(std::unique_ptr<T>(
          new T(nic_.get(), qp_pool_cnt_, qp_pool_cnt_)));
      ++qp_pool_cnt_;
      return qp_pool.back().get();
    } else {
 ...
 private:
  ...
  /// Rpc client pool.
  std::vector<std::unique_ptr<T>> qp_pool;
