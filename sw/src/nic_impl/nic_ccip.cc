...
int NicCCIP::open_connection(ConnectionId& c_id, const IPv4& dest_addr,
                             ConnectionFlowId c_flow_id, uint16_t remote_qp_num,
                             uint16_t p_key, uint32_t q_key) const {
  std::unique_lock<std::mutex> lck(conn_setup_mtx_);

  if (conn_manager_.open_connection(c_id, dest_addr, c_flow_id, remote_qp_num,
                                    p_key, q_key) != 0) {
    FRPC_ERROR("Failed to open connection\n");
    return 1;
  }

  if (register_connection(c_id, dest_addr, c_flow_id, remote_qp_num, p_key,
                          q_key) != 0) {
...
int NicCCIP::add_connection(ConnectionId c_id, const IPv4& dest_addr,
  ...
  if (conn_manager_.add_connection(c_id, dest_addr, c_flow_id, remote_qp_num,
                                   p_key, q_key) != 0) {
...
int NicCCIP::register_connection(ConnectionId c_id, const IPv4& dest_addr,
                                 ...
                                 uint16_t remote_qp_num, uint16_t p_key,
  ...
  ConnSetupFrame frame = {.data = c_id, .cmd = setUpConnId, .big_data = 0};
  ...
  // setUpOpen
  frame = {.data = cOpen, .cmd = setUpOpen, .big_data = 0};
  res = fpgaWriteMMIO64(accel_handle_, 0, base_nic_addr_ + iRegConnSetupFrame,
                        *reinterpret_cast<uint64_t*>(&frame));
  if (res != FPGA_OK) {
    FRPC_ERROR(
  ...
  // setUpDestIPv4
  frame = {.data = dest_addr.get_addr(), .cmd = setUpDestIPv4, .big_data = 0};
  res = fpgaWriteMMIO64(accel_handle_, 0, base_nic_addr_ + iRegConnSetupFrame,
                        *reinterpret_cast<uint64_t*>(&frame));
  if (res != FPGA_OK) {
    FRPC_ERROR(
  ...
  // setUpDestPort
  frame = {.data = dest_addr.get_port(), .cmd = setUpDestPort, .big_data = 0};
  res = fpgaWriteMMIO64(accel_handle_, 0, base_nic_addr_ + iRegConnSetupFrame,
                        *reinterpret_cast<uint64_t*>(&frame));
  if (res != FPGA_OK) {
    FRPC_ERROR(
  ...
  // setUpClientFlowId
  frame = {.data = c_flow_id, .cmd = setUpClientFlowId, .big_data = 0};
  res = fpgaWriteMMIO64(accel_handle_, 0, base_nic_addr_ + iRegConnSetupFrame,
                        *reinterpret_cast<uint64_t*>(&frame));
  if (res != FPGA_OK) {
    FRPC_ERROR(
  ...
  //setUpQPFields
  frame = {.data = 0, .cmd = setUpQPFields, .big_data = qp_fields};
  res = fpgaWriteMMIO64(accel_handle_, 0, base_nic_addr_ + iRegConnSetupFrame,
                        *reinterpret_cast<uint64_t*>(&frame));
  if (res != FPGA_OK) {
    FRPC_ERROR(
  ...
  // setUpEnable
  frame = {.data = 1, .cmd = setUpEnable, .big_data = 0};
  res = fpgaWriteMMIO64(accel_handle_, 0, base_nic_addr_ + iRegConnSetupFrame,
                        *reinterpret_cast<uint64_t*>(&frame));
  if (res != FPGA_OK) {
    FRPC_ERROR(
...
int NicCCIP::remove_connection(ConnectionId c_id) const {
  ...
  // setUpOpen
  frame = {.data = cClose, .cmd = setUpOpen, .big_data = 0};
  res = fpgaWriteMMIO64(accel_handle_, 0, base_nic_addr_ + iRegConnSetupFrame,
                        *reinterpret_cast<uint64_t*>(&frame));
  if (res != FPGA_OK) {
    FRPC_ERROR(
...
volatile void* NicCCIP::alloc_buffer(fpga_handle accel_handle, ssize_t size,
  ...
  *io_addr = *io_addr + aa_offset_bytes;
  buf = reinterpret_cast<volatile uint8_t*>(buf) + aa_offset_bytes;
