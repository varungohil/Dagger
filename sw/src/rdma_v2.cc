...
RDMA::RDMA(uint64_t base_nic_addr, size_t max_num_of_threads, size_t max_qp_pool_size)
...
int RDMA::check_hw_errors() const {
  return nic_->check_hw_errors();
}

int RDMA::run_new_listening_thread(
...
int RDMA::make_qp(){
  if (qp_pool_cnt_ < max_qp_pool_size_) {
    qp_pool_.emplace_back(std::unique_ptr<QueuePair>(new QueuePair(
        nic_.get(), qp_pool_cnt_, qp_pool_cnt_))); 
    ++qp_pool_cnt_;
    return 0;
  } else {
...
int RDMA::send(uint16_t queue_pair_num){
    ...
    qp_pool_cnt_[queue_pair_num].send();
}
